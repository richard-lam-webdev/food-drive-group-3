...
export default function Accueil() {
  ...
  const [productsByCategory, setProductsByCategory] = useState<any>({}); // Produits regroupés par catégorie
  const carouselRefs = useRef<any[]>([]); // Tableau de références pour les carrousels
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      if (res.ok) {
        ...
        setProducts(data);
        groupProductsByCategory(data); // Regrouper les produits par catégories
      } else {
  ...
  const groupProductsByCategory = (products: any[]) => {
    ...
    carouselRefs.current = Object.keys(grouped).map(() => React.createRef()); // Initialiser les références
  };
  ...
  return (
    <section className="py-16 bg-green-50">
      ...
      <p className="text-center text-gray-600 mb-12">
        Parcourez nos sélections soigneusement choisies pour répondre à tous vos
